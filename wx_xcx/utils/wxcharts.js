...
function drawXAxis(categories, opts, config, context) {
    ...
    if (config._xAxisTextAngle_ === 0) {
        ...
        categories.forEach(function (item, index) {
            let items = item.split(' ');
            var offset = eachSpacing / 2 - measureText(items[0]) / 2;
            context.fillText(items[0], xAxisPoints[index] + offset, startY + config.fontSize + 5);
        });
        context.closePath();
        context.stroke();
    } else {
        categories.forEach(function (item, index) {
            ...
            context.setFillStyle(opts.xAxis.fontColor || '#666666');
            let items = item.split(' ');
            var textWidth = measureText(items[0]);
        ...
        case 'area':
            this.animationInstance = new Animation({
                ...
                onProcess: function onProcess(process) {
                    ...
                    _this.chartData.calPoints = calPoints;
                    _this.chartData.eachSpacing = eachSpacing;
                    _this.getDataWidth(_this.chartData,opts,process);
                    drawXAxis(categories, opts, config, context);
                    drawLegend(opts.series, opts, config, context);
                    drawYAxis(series, opts, config, context);
                    drawToolTipBridge(opts, config, context, process);
...
var Charts = function Charts(opts) {
    ...
    // such as chart point coordinate1
    this.chartData = {};
...
Charts.prototype.getDataWidth = function (chartData,opts,process) {
    var that = this;
    if(process==1) return;
    var dataChartAreaWidth = opts.width - config.padding - chartData.xAxisPoints[0];
    var dataChartWidth = chartData.eachSpacing * opts.categories.length;
    that.scrollOption.currentOffset = -(dataChartWidth-dataChartAreaWidth);
    that.opts._scrollDistance_ = -(dataChartWidth-dataChartAreaWidth);
};
...
Charts.prototype.scrollEnd = function (e) {
    if (this.opts.enableScroll === true) {
        ...
        this.scrollOption.currentOffset = currentOffset + distance;/*这里有数据驱动视图*/
